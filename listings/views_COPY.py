...
from .forms import UploadEproofForm, UploadDocumentForm, EditDocumentForm
...
def edit_document(request, listing_id):
    selected_document = get_object_or_404(Listing, pk=listing_id)
    form = EditDocumentForm(instance=selected_document)

    if request.method == 'POST':
        form = EditDocumentForm(request.POST, request.FILES, instance=selected_document)
        if form.is_valid():
...
def upload_quotation(request):
    ...
    if request.method == 'POST':
        ...
        form = UploadDocumentForm(initial=initial_data)
